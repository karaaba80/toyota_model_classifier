...
def load_yolo_once():
    ...
    with torch.no_grad():
        ...
        # device = get_device()
        print ("device for yolo:", get_device())
...
def get_bboxes_yolo(yolo_model, image_obj, min_wh2image_ratio=0.2, min_obj_conf=0.9):
    ...
    bboxess_probs = get_bboxes_xywh_with_class_filters(detections, yolo_class_names, class_filters=["car"])  # we want only "car" obj
...
def detect_predict_and_show(img_obj, image_placeholder):
    ...
    # print("bboxes_probs", bboxes_probs)
    print("img_obj_crop", type(img_obj_crop))
    if not img_obj_crop==None:
       np_value, label, conf = predict_image_object(img_object=img_obj_crop, model=model, labels=classes, res=resolution)
