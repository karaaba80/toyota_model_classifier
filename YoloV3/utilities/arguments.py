...
def parse_detect_args():
    ...
    parser.add_argument("--input-dir", type=str, default="", help="Image directory")

    parser.add_argument("-output", type=str, default="./detections.png", help="Output image file (video out if --video)")

    parser.add_argument("--output-dir", type=str, help="output image directory", default="")
    ...
    parser.add_argument("-cfg", type=str, default="./configs/yolov3.cfg", help="Yolo configuration file")
    parser.add_argument("-weights", type=str, default="./weights/yolov3.weights", help="Yolo weights file")

    parser.add_argument("-class_names", type=str, default="./configs/coco.names", help="Names for each class index")

    parser.add_argument("--class-filters", type=str, default=None, help="classes to be shown, if none, it means everything") #faik added

    parser.add_argument("--obj-thresh", type=float, default=OBJ_THRESH_DEF, help="Confidence threshold for filtering out predictions")
    parser.add_argument("--letterbox", action="store_true", help="Turns on image input letterboxing")

    parser.add_argument("--min-scale",  dest="min_scale", default="224x224", help="Specifies minimum resolution of the image for input")

    parser.add_argument("--no_show", action="store_true", help="Does not display output image with detections (ignored if --video)")
    parser.add_argument("--no-save", action="store_true", help="Does not display output image with detections (ignored if --video)")
